...
*/
{
  return (int_path_length(root,0)/num_nodes);
}
...
int AvlTree::int_path_length(AvlNode *t, int depth) {
  ...
  if(t->left==NULL && t->right==NULL){ //only root node
      return d;
    }
    else{
      if (t->left != NULL){  //need to add number of nodes on left
	lft+=int_path_length(t->left, depth+1);
	d+=depth;
      }
      if(t->right!=NULL){   //add number of nodes on left
	rt+= int_path_length(t->right,depth+1);
	d+=depth;
      }
      return (d+rt+lft);  //return total
    }
}
